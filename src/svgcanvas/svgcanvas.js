...
import {
  ...
  getIntersectionListMethod,
  getMouseTargetMethod,
  groupSvgElem,
  init as selectionInit,
  prepareSvg,
...
import {convertToNum, getTypeMap, init as unitsInit} from '../common/units.js'
import {
  convertGradientsMethod,
  embedImage,
  exportPDF,
  importSvgString,
  init as svgInit,
  rasterExport,
  removeUnusedDefElemsMethod,
  setSvgString,
  setUseDataMethod,
  svgCanvasToString,
  svgToString,
...
import {init as coordsInit, remapElement} from './coords.js'
import {init as recalculateInit, recalculateDimensions} from './recalculate.js'
import {getSelectorManager, init as selectInit, Selector} from './select.js'
...
const refAttrs = ['clip-path', 'fill', 'filter', 'marker-end', 'marker-mid',
...
class SvgCanvas {
  ...
  constructor(container, config) {
    ...
    if (config) {
      this.curConfig = mergeDeep(this.curConfig, config)
    }
    ...
    const allProperties = {
      shape: {
        fill: (this.curConfig.initFill.color === 'none' ? '' : '#')
    ...
    allProperties.text = mergeDeep({}, allProperties.shape)
    allProperties.text = mergeDeep(allProperties.text, {
      fill: '#000000',
    ...
    const storageChange = (ev) => {
      if (!ev.newValue) {
        return
      } // This is a call from removeItem.
  ...
  setMode(name) {
    ...
    this.curProperties = (this.selectedElements[0]
      && this.selectedElements[0].nodeName === 'text') ? this.curText
      : this.curShape
  ...
  async addExtension(name, extInitFunc, {importLocale}) {
    ...
    if (name in this.extensions) {
      throw new Error('Cannot add extension "' + name
  ...
  removeFromSelection(elemsToRemove) {
    if (isNullish(this.selectedElements[0])) {
      return
    }
  ...
  convertToPath(elem, getBBox) {
    ...
    if (getBBox) {
      return getBBoxOfElementAsPath(elem, this.addSVGElementsFromJson,
        this.pathActions)
    }
